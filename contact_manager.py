...
class ContactManager(object):#this class creates a database 
	...
	def add_record(self, name, number):#populates the database table with required values
		conn = sqlite3.connect('contacts_v1.db')
...
def send_text(name, message):
	...
	if not len(data):
		return 'Kindly add the contact you want to send a message to : '
	elif len(data) > 1:
		...
		if response > results[len(results) - 1][0] or response <= 0:
			...
			theNumber = new_cursor.fetchall()
			print "\nSending message to %s %s" %(name,theNumber[0][0]) + "..."

			print theNumber[0][0]
			send_message(theNumber[0][0], message)
	else:
			to_number = data[0][2]
			print "\nSending message to %s %s" %(name, int (to_number))
